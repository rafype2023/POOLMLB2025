document.addEventListener("DOMContentLoaded", function () {
  // --- 1. CONFIGURATION ---
  const alSeeds = ["Seattle Mariners", "Toronto BlueJays", "Cleveland Guardians", "Seattle Mariners", "Houston Astros", "Minnesota Twins"];
  const nlSeeds = ["Philadelphia Phillies", "Milwaukee Brewers", "Philadelphia Phillies", "Cincinnati Reds", "Chicago Cubs", "San Diego Padres"];
  ...
  function populateMatchup(wrapperId, selectId, team1, team2, maxGames) {
    ...
    `;
    if (winners[selectId]) wrapper.querySelector(`#${selectId}`).value = winners[selectId];
    if (seriesLengths[selectId]) wrapper.querySelector(`#${selectId}-length`).value = seriesLengths[selectId];
  }
  ...
  function validateForm() {
    ...
    const seriesIds = [ 'al-wc1', 'al-wc2', 'nl-wc1', 'nl-wc2', 'al-ds1', 'al-ds2', 'nl-ds1', 'nl-ds2', 'al-cs', 'nl-cs', 'ws-winner' ];
